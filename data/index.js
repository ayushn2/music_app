export const data = {
    events: [
        {
            id: 1,
            date: {
                day: 15,
                month: "Feb"
            },
            location: {
                city: "Los Angeles",
                country: "USA",
                address: "LA Convention Center"
            },
            priceRange: "60-250$"
        },
        {
            id: 2,
            date: {
                day: 20,
                month: "March"
            },
            location: {
                city: "Rio de Janeiro",
                country: "Brazil",
                address: "Sambadrome"
            },
            priceRange: "40-200$"
        },
        {
            id: 3,
            date: {
                day: 10,
                month: "May"
            },
            location: {
                city: "Paris",
                country: "France",
                address: "Eiffel Tower Park"
            },
            priceRange: "70-300$"
        },
        {
            id: 4,
            date: {
                day: 30,
                month: "May"
            },
            location: {
                city: "Berlin",
                country: "Germany",
                address: "Berlin Exhibition Hall"
            },
            priceRange: "55-280$"
        },
        {
            id: 5,
            date: {
                day: 6,
                month: "August"
            },
            location: {
                city: "Athens",
                country: "Greece",
                address: "Acropolis Amphitheatre"
            },
            priceRange: "45-220$"
        },
        {
            id: 6,
            date: {
                day: 12,
                month: "Sept"
            },
            location: {
                city: "Bucharest",
                country: "Romania",
                address: "National Arena"
            },
            priceRange: "42-260$"
        },
        {
            id: 7,
            date: {
                day: 3,
                month: "Oct"
            },
            location: {
                city: "Warsaw",
                country: "Poland",
                address: "PGE Narodowy"
            },
            priceRange: "46-290$"
        }
    ],
    albums : [
        {
            id: 1,
            img: "/assets/albums/cover/cover-1.png",
            name: " 1",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        },
        {
            id: 2,
            img: "/assets/albums/cover/cover-2.png",
            name: " 2",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        },
        {
            id: 3,
            img: "/assets/albums/cover/cover-3.png",
            name: " 3",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        },
        {
            id: 4,
            img: "/assets/albums/cover/cover-4.png",
            name: " 4",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        },
        {
            id: 5,
            img: "/assets/albums/cover/cover-5.png",
            name: " 5",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        },
        {
            id: 6,
            img: "/assets/albums/cover/cover-6.png",
            name: " 6",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        },
        {
            id: 7,
            img: "/assets/albums/cover/cover-7.png",
            name: " 7",
            tracks: [
                {
                    name: "Serenade in D Major",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Midnight Lullaby",
                    src: "assets/f        reedom.mp3"
                },
                {
                    name: "Whispers of the Wind",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Dance of the Fireflies",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Echoes of the Past",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Enchanted Dreams",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Sunlight Serenity",
                    src: "assets/freedom.mp3"
                },
                {
                    name: "Misty Moonlight Melody",
                    src: "assets/freedom.mp3"
                }
            ]
        }
    ],
    
    
    posts: [
        {
            id: 1,
            date: "2023-09-21",
            title: "Rock Concert at Central Park",
            description: "Join us for an electrifying night of classic hits at Central Park! The lineup includes tribute bands for legendary rock icons."
        },
        {
            id: 2,
            date: "2023-09-21",
            title: "Jazz Jam session",
            description: "Join us for an electrifying night of classic hits at Central Park! The lineup includes tribute bands for legendary rock icons."
        },
        {
            id: 3,
            date: "2023-09-23",
            title: "EDM Extravanganza",
            description: "Join us for an electrifying night of classic hits at Central Park! The lineup includes tribute bands for legendary rock icons."
        },
        {
            id: 4,
            date: "2023-09-27",
            title: "Acoustic",
            description: "Join us for an electrifying night of classic hits at Central Park! The lineup includes tribute bands for legendary rock icons."
        },
        {
            id: 5,
            date: "2023-09-21",
            title: "Classical Symphony Showcase",
            description: "Join us for an electrifying night of classic hits at Central Park! The lineup includes tribute bands for legendary rock icons."
        },
        {
            id: 6,
            date: "2023-09-21",
            title: "Hip-Hop Dance Battle",
            description: "Join us for an electrifying night of classic hits at Central Park! The lineup includes tribute bands for legendary rock icons."
        }
    ]
};

// Example access:
console.log(data.events[0].location.city); // Outputs: Los Angeles
